...
public class TimeEntryControllerTest {
    ...
    public void testCreate() throws Exception {
        long projectId = 123L;
        long userId = 456L;
        TimeEntry timeEntryToCreate = TimeEntryBuilder.builder().projectId(projectId).userId(userId).date(LocalDate.parse("2017-01-08")).hours(8).build();

        long timeEntryId = 1L;
        TimeEntry expectedResult = TimeEntryBuilder.builder().id(timeEntryId).projectId(projectId).userId(userId).date(LocalDate.parse("2017-01-08")).hours(8).build();
    ...
    public void testList() throws Exception {
        List<TimeEntry> expected = asList(
                ...
                TimeEntryBuilder.builder().id(2L).projectId(789L).userId(321L).date(LocalDate.parse("2017-01-07")).hours(4).build()
        );
    ...
    public void testUpdate() throws Exception {
        ...
        long projectId = 987L;
        long userId = 654L;
        TimeEntry expected = TimeEntryBuilder.builder().id(timeEntryId).projectId(projectId).userId(userId).date(LocalDate.parse("2017-01-07")).hours(4).build();
        doReturn(expected)
            .when(timeEntryRepository)
    ...
    public void testUpdate_NotFound() throws Exception {
        ...
        ResponseEntity response = controller.update(nonExistentTimeEntryId, TimeEntryBuilder.builder().build());
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
    }
